...
One method that can increase verification efficiency is the use of various types of coverage measures. As such, we explore how existing tools can enable statement coverage of a Chisel design at Scala and Verilog levels. Furthermore, we create a new method allowing for statement coverage at the FIRRTL level. Finally, we describe our solution for gathering functional coverage on a Chisel design.
We evaluate our approach with a real-life design.
...
Figure~\ref{fig:chisel} shows an overview of the Chisel compilation pipeline and the different stages the code goes through before being converted into a usable format. \todo{Perhaps replace with the new figure from the ChiselVerify paper?}
...
Expression coverage takes this one step further checking whether all possible combinations of true checks are executed. The left code snippet in Figure~\ref{fig:expr} shows an example of path coverage, while the right code snippet shows expression coverage. One must note that the finer the granularity, the greater the computational requirements. For simple designs, statement coverage is generally sufficient~\cite{hdlverify}. \hjd{Check up on this and explain more.}
...
A sample of the ports is considered a hit if all of the port values are contained in their associated ranges. \todo{Expand on usefulness of this measure.} %A hit is then considered if each of the ports was sampled with a value that is contained in its associated range.  
...
============================================
\end{verbatim}
The above report shows each point with its user-defined name and the bins it contains. These are then associated to a number of hits and, whenever possible \todo{Add explaining sentence here.}, a coverage percentage.
...
This shows that our coverage solution is on-par with existing coverage solutions, even being more concise and introducing new functionalities like purely conditional coverage, which is only possible using Scala's functional nature. \todo{Perhaps re-write this sentence to make it clear that it is \textit{only} the conditional coverage that requires Scala's functional nature.}
...
\martin{There is an anonymous Git  possible. We have the pointer in one of the reviews.}

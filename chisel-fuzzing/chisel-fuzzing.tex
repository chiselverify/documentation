...
\martin{I do not understand the following sentence. It is also a bit too long.}
...
\martin{Following: why do we increase the efficiency of verification when the HW design is time-consuming?}
...
\martin{General question: why using ``RTL design'' and not just ``hardware design'', or ``digital design''?}
This allows for optimal fuzzing for RTL designs using functional coverage as a metric to drive the fuzzing.

This paper describes a research project that aims to develop an optimal \martin{optimal is a very ambitious aim; maybe not achievable. When do we know when it is optimal?} functional coverage-driven mutation-based fuzzing tool to test RTL circuits.
...
\martin{Really VHDL and Verilog?}
...
% \martin{Great then there is room for us to develop further :-)}
...
\martin{General: avoid ``very'' in technical writing. Also avoid isn't and other more slang like terms.}
\martin{Andrew, feel free to remove my comments when you read it. To keep the paper somehow clean.}
...
\martin{Random ``program'' generation comes here as a surprise. Up to now the reader did not know that we generate programs. Tjark, maybe you can add this into the introduction.}

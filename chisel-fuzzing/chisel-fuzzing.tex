...
Functional coverage is a hardware-oriented coverage metric that helps verify is certain features of a given specification have been tested thoroughly.
These features are defined in what's called a \textit{verification plan}~\cite{spear2008systemverilog}.
This is defined using a set of \texttt{cover} constructs. 
...
A \texttt{hit} is considered when a value is sampled for the port that is either contained in the bin's range or validates its condition.

...
Given the complexity of UVM and SystemVerilog combined with the need for proprietary development tools, the industry is lacking and continuously seeking for verification engineers that have experience in the universal verification methodology. For these reasons, alternative solutions inspired by the UVM methodology were born. This section contains a list of projects that aim to achieve functional verification of a design in a more accessible way.
\subsection{CoCoTb}
CoCoTb (COroutine based COsimulationTestBench) is a python library that aims to provide an environment for verifying VHDL and SystemVerilog RTL designs. Following the philosophy of UVM, CoCoTb seeks design reusability and constrained randomization tests. In contrast with UVM, test benches are written directly in python, which lowers the time spent in verification and development. CoCoTbe leverages the VPI (Verilog Programming Interface) or VHPI (VHDL Programming Interface) to communicate with the most common RTL simulators. Through this standard set of API, CoCotb can communicate with the kernel of the simulator and control events and signal values during a simulation. \cite{cocotb2020Sep}
Moreover, given the flexibility of the Python language and the large amount of library available for it, it is easy to integrate regression tests of digital design in a CI/CD environment. A remarkable extension of CoCoTb is python-uvm, which is a complete port of UVM 1.2 to python \cite{tpoikela2020Oct}.

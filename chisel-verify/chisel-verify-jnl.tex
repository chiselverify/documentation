...
It is enabled by extensions to the Python \hjd{missing word here?}, that allow for coroutine support and DUT interfacing using a very simple \texttt{dut.port.value} interface.
...
In order to abstract interaction with the DUT and provide an interface at the same abstraction level as the reference model, we employ BFM-like wrappers. 
...
To do so, we use our baseline a bare ChiselTest test bench without any functional coverage or CRV.
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{dobis.jpeg}}]{Andrew Dobis}
is a Masters student in Computer Science at ETH Zurich. 
After obtaining his Bachelors degree in the same field from EPFL, he worked at the Technical University of Denmark's Embedded systems group as a Research Assistant.
His research focused on developing improving the verification capabilities of the Chisel Hardware Construction language, which led to work on the ChiselVerify verification library.

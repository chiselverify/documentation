...
Arbiter_Coverage = coverage_section(
    CoverPoint("top.io_out_ready", vname="io_out_ready", rel = range_relation, 
        bins = [0, 1], bins_labels = ["ready0", "ready1"]),
    CoverPoint("top.io_out_valid", vname="top.io_out_valid", rel = range_relation, 
        bins = [0, 1], bins_labels = ["valid0", "valid1"]),
    # [...] Repeat for each port
)
 \end{lstlisting}
 Here the range relation lambda tells cocotb how to interpret the bounds of a range. 
 This can be compared to how ChiselVerify allows for custom hit conditions to be defined.
 When a single number is give, e.g.\ $0$, this is interpreted as a range \texttt{0 to 0}, thus a hit is considered if our relation holds for the sampled value, i.e.\ \texttt{0 <= val <= 0}.
 Only considering coverage-related code, the verification plan defined in cocotb requires 82 lines of cocotb-coverage code.
...
For example, in the case of the arbiter, which has a variable number of inputs, we can simply define our verification plan using a generic loop over all inputs in the design as follows:
 \begin{lstlisting}[language=scala]
dut.io.in.zipWithIndex.foreach((input: DecoupledIO[UInt], idx: Int) => {
  cr.register(
    cover(s"in${idx}.ready", input.ready)(
        bin("Ready0", 0 to 0), 
        bin("Ready1", 1 to 1)),
 ...
 \end{lstlisting}
Here, the \texttt{zipWithIndex} call allows us to concisely loop over our array with a \texttt{foreach} loop, while having access to the loop index, enabling the creation of unique identifiers for our \texttt{cover} objects.
...
It is important to keep in mind that these user-level comparisons are not absolute, and are used to give the reader an idea of the amount of code that is required to be written in the different languages to solve the same problems.
...
%\todo{KASPER: REPLACE WITH YOUR MACHINE'S SPECS}
\begin{table}[h]
\centering
\begin{tabular}{|l|c|}
\hline
\multicolumn{1}{|c|}{\textbf{}}      & \textbf{Processor}     \\ \hline
\textbf{Model} & Intel Core m5-6Y54 CPU \\ \hline
\textbf{Microarchitecture}                       & Skylake                \\ \hline
...
%\todo{KASPER: REPLACE WITH YOUR RESULTS FOR CHISELVERIFY}
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{bios/ducky.jpg}}]{Richard Lin}

...
We have selected UVM since it is an industry standard that can be used to verify Verilog designs, an \texttt{cocotb} since it enables similar verification functionalities in a high-level environment like what we propose with ChiselVerify.
The results are then compared in terms of the verbosity of the verification code, and runtime on similar tests.
As a final evaluation, we look at the overhead caused by the use of ChiselVerify specific functionalities, such as functional coverage and CRV, by comparing the runtime of a bare ChiselTest testbench, to the same one enhanced with our verification tools.

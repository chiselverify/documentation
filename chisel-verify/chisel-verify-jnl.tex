...
This is enables by extension to the python, that allow for coroutine support and DUT interfacing using a very simple \texttt{dut.port.value} interface.
Cocotb was also extended with a library called cocotb-coverage, which allows for functional coverage and constrained random verification to be added to the python-based testbenches.
This solution follows the same goals as ours, since it aims to improve verification efficiency by allows for verification to take place in a high-level environment.
However, our solution aims to do so in a way that is well integrate with the design flow, by allow for a high-level design to be verified in an equally high-level environment.

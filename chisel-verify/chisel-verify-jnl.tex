...
% Our system always implicitly adds the assumption that during the first cycle of the formal check,
% the reset pin will be asserted. This is similar to how the dynamic testing interface will push the
% circuit through a reset cycle before executing the user test.
% By default assertions in Chisel are guarded by the reset signal.
% The automatic reset behavior can be disabled for special cases, however, since Chisel normally
% mandates reset conventions, the automatic assumptions will cover the majority of use-cases.

...
This model can be seen as a series of constraints which can then be given to an SMT solver, such as Z3~\cite{de2008z3}, which returns whether or not a set of inputs exists that meets the given constraints.
...
\begin{figure*}
  \centering
    ...
    \caption{Overview of the Chisel compilation pipeline. \martin{redraw for copyright issues} \martin{The bottom flow (Verilator) should also go from left to right. It is not a feedback loop.}}

...
In order to evaluate the different verification tools provided by ChiselVerify, we will be using two different example circuits, namely an arbiter circuit and a priority queue provided by Microchip~\cite{microchip}.
These are tested using ChiselTest test-benches augmented with our verification tools.
The test-benches are then compared to similar tests writing using UVM and \texttt{cocotb}, using the generated Verilog description as a DUT.
...
We evaluate ChiselVerify on two circuits, which we describe in the following paragraphs.
...
\paragraph{Functional Coverage in cocotb} %TODO
\paragraph{Functional Coverage with ChiselVerify} %TODO
...
\begin{figure}[h]
  ...
  \caption{Verbosity overhead comparison between verification plans written for two designs with ChiselVerify (CV), SystemVerilog (SV), and cocotb (PY).}
  \label{fig:cvsvpycomp}
  \end{figure}
  This shows that, with ChiselVerify, we obtain a 83\% reduction in LOC on the Arbiter circuit over UVM and a 70\% reduction over cocotb. 
  This is due, in this case, to ChiselVerify's capability of exploiting the high-level design constructs, such as Vectors, directly in the Verification Plan, while the other solutions need to rely on the unfolded generated Verilog.
  For the Priority Queue, we obtain a 60\% reduction over UVM, and an increase of 30\% over cocotb. 
  This increase comes from cocotb's more concise way of declaring bins in a coverpoint.
  However, this is a somewhat incomplete comparison, since cocotb does not provide the tools to express value transitions or temporal relations inside of a cover point, making its VP a less accurate model of our specification.

...
This framework is inspired by the Universal Verification Method (UVM) with SystemVerilog~\cite{SystemVerilog}, but leverages Scala's conciseness with the
...
SystemVerilog is an extension of Verilog~~\cite{SystemVerilog}.
Many non-synthesizable extension are intended
...
As far as we know, ChiselVerify is the only verification framework allowing for the simple use of verification functionalities, well integrated into the ChiselTest/Chisel ecosystem.
...
TimedCross("timed_valid","valid","valid",
  Eventually(10))(
    CrossBin("revoked valid under operation",
      1 to 1, 0 to 0) :: Nil
)
\end{lstlisting}

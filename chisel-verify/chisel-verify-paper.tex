...
\section{Verification of AXI Interfaced Components}
Another solution to the ever-increasing complexity of digital designs is to use standardized interfaces -- this also enables greater reuse. One of such standard interfaces is AXI -- an open standard by ARM \cite{}, which is used in particular to connect processor nodes to memories over an unspecified interconnect (i.e., one is allowed to choose between a bus or a NoC). As such, most available synthesis tools, including Xilinx' Vivado, provides IP generators whose output IP blocks are equipped with AXI interfaces along with optional verification structures written in (System-)Verilog \cite{}. Typically, verification of components with such standard interfaces is provided through so-called \textit{bus functional models} (BFMs) that abstract complex low-level signal transitions between bus masters and slaves to a transaction level (e.g., write and read transactions). Unfortunately, such BFMs are not yet available in Chisel -- hence, why we include an example BFM in our framework.

\subsection{Introduction to AXI}


\subsection{Implemented Framework}


\todo{Get Hans to add his stuff here} \hjd{On it!}

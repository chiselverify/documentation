...
This paper is an extension of \cite{verify:chisel:2020}, which discussed different approaches towards enabling verification directly in Chisel.
...
We propose, as a solution to the Chisel's lack of verification-oriented features, ChiselVerify, a verification library based entirely in Scala. ChiselVerify uses the DUT interfacing features from ChiselTest in order to enable three main verification functionalities in Chisel, namely Functional Coverage, Constraint Random Verification and Bus Functional Modelling. We also show how our framework can be used to create a Bus Functional Model, by creating an example one for the standardized AXI4 interface. The following subsections will give a deep-dive into each one of the functionalities and present how to use them.
...
Supporting a coverage measure directly in the testing tool enables modern verification strategies such as constraint random verification, that we will now describe our implementation of.

\subsection{Constraint Random Verification}
As described earlier, the complexity of digital design is increasing with the capacity of the silicon. Traditionally, verification engineers would write a number of directed tests each focusing on one or a few of a components functionalities, but this approach becomes impractical for larger designs. Instead, a Monte Carlo-style randomized verification methodology may be applied; constraint random verification, which is a combined effort of constraint programming and coverage-based methodologies. Using constraint random inputs, a relatively small test suite can, statistically, cover many of a component's functionalities. The constraints help to ensure that no invalid input combinations that would not appear during regular operation are applied \hjd{Needs citation}.
...
Our implementation supports random variables of the three main Chisel data types; integers, Booleans, and bundles, i.e., combinations of signals of the same or different types. Each variable has a specific domain of values that it is constrained to by the user, and values within the domain are equally likely to be selected by the CSP solver. Random objects are defined by extending a base class \texttt{Random} included in the library. Afterwards, fields of the object that are to be randomized must be declared of type \texttt{RandInt} and initialized with the \texttt{rand} macro. Finally, inherited from the \texttt{Random} class is the \texttt{random} method, which may be called to randomize the object's fields.

\subsubsection{An Example Random Object}
...
%As it can be seen in figure~\ref{fig:pq_control_state}, there are a series of special cases where insertion or removal times can be reduced for instance by taking advantage of the head element being stored in a register. 
%The achieved best and worst case insertion as well as removal times are presented in table~\ref{tab:pq_timings}.
%
%\begin{figure}
%	\centering

...
The above code snippet is an example of how to define a verification plan using our coverage framework. One concept used in the example verification plan is the idea of \textit{cross coverage} defined using the \texttt{Cross} construct. Cross coverage allows one to specify coverage relations between \texttt{CoverPoint}s. This means that cross-defined between, e.g., \texttt{CoverPoint} $a$ and \texttt{CoverPoint} $b$ will be used to gather information about when $a$ and $b$ have specific values simultaneously~\cite{spear2008systemverilog}.
...
In this work, we provide an example BFM for AXI4, an open standard by ARM~\cite{axi4standard}. The example works only for AXI4, but it can easily be extended to support other buses. So, let us first give a brief introduction to AXI4.

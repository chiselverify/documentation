...
\textit{UC Berkeley}\\
Berkeley, CA \\\\
adobis@student.ethz.ch, s186083@student.dtu.dk, s183735@student.dtu.dk, s190057@student.dtu.dk, \\
hans.damsgaard@tuni.fi, simon.thye@gmail.com, richard.lin@berkeley.edu, masca@dtu.dk}
}
...
efficient tools for verification. %is the lack of modern tools to attack it. To verify a design in such a time-constrained development
%method, one needs to have efficient tools both for design and verification.

Chisel, a high-level hardware construction language, was introduce in order to combat this lack.
...
This approach is similar to software contracts in Scala, like the ones enabled by ScalaCheck~\todo{cite scala check}. 
...
We will also talk about Chisel and the current solutions that exist in it with regards to the verification of digital designs.
...
\begin{itemize}
  \item \texttt{Bin}: Ranges of values that should be tested for (i.e., expected values of a given port).
  \item \texttt{Cover} constuct: Port that needs to be sampled in the coverage report, defined using a set of \texttt{bin}s.
\end{itemize}
...
Constrained Random Verification allows the verification engineer to create random variables which generated values that verify a set of associated constraints.
...
A Bus Functional Model is an abstract model of a standardized interface, or a bus of multiple signals, that can be interacted with using transactions. 
Many synthesis tools, including Xilinx's Vivado, provide IP generators whose output IP blocks are equipped with such standard interfaces. 
...
This thus enables simpler and safer less-verbose interactions with standardized interfaces like I2C or AXI.
...
The Advanced eXtensible Interface (AXI) protocol by ARM is a highly flexible interconnect standard based around five independent channels; three for write operations and two for read operations. 
Operations, known as transactions, consist of several transfers across either set of channels. 
All channels share a common clock and active-low reset, and base their transfers on classic ready-valid handshaking. 
It is designed with memories in focus that supports multiple outstanding transactions and out-of-order completion. 

Consider, for example, a write transaction of 16 data elements. 
First, the manager provides transaction attributes (e.g., target address and data size) as a single transfer over the \textit{Write Address} channel, then the manager transfers the 16 data elements one at a time over the \textit{Write Data} channel, and finally, the subordinate indicates the status of the transaction over the \textit{Write Response} channel. 
The \textit{Read Address} and \textit{Read Data} channels may operate independently at the same time. 
A full description is available in the AXI4 stanrdard~\cite{axi4standard}.
...
By increasing the number of children $k$ per node, the depth, as well the run-times, can be reduced.
...
\begin{lstlisting}[language=scala, caption={A timed cover construct.}, label={lst:axitest}]
...
For comparison, our Scala functional coverage took 12 lines to define.
Its UVM counterpart took 84 lines, only including the \texttt{uvm\_subscriber} class.
Using our Scala-based solution, we have minimized the amount of code needed to use these functionalities. 
For functional coverage, all that needs to be done is to define the verification plan and sample it.
CRV can be done by only defining the random object. 

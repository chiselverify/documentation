...
\begin{itemize}
\item REVIEW 1: Re-write abstract and introduction - remove anything not related to verification. Remove negative wording on Verilog and VHDL. Re-consider necessity of section III.A and Fig. 2. Switch to Manager/Subordinate terminology. Need for a more advanced use case.
...
\item REVIEW 3: Prove claimed productivity increase. Re-write reference to "Stuart Russel's book". 
\item REVIEW 4: Need for a more advanced use case.
\item REVIEW 5: Prove claimed productivity increase. Add table of available methods (section III). Remove Moore's law statements.
\end{itemize}
...
The main contribution of this paper is ChiselVerify~\footnote{Link removed for anonymity.}, an open-source verification library for hardware designs.
...
before, included in SystemVerilog, and we provide another implementation in ChiselVerify. An initial attempt at 
implementing a custom CSP solver in Scala based on backtracing and arc consistency, as described in Stuart Russel's book on modern AI~
\cite{russell2002artificial}, performed poorly. Instead, ChiselVerify provides a wrapper to an existing CSP solver, named JaCoP~\cite{jacop2013}, and 
...
In our research, we received a use case from Microchip in the form of a specification.

...
\end{itemize}
It is important to note that this information needs to be interpreted correctly to be of any use. A more interesting result would be if the FIRRTL line coverage would be mapped to the original Chisel source. This is possible but challenging, since Treadle only has access to the original source code through \textit{Source locators} which map certain FIRRTL lines (only those that don't contain multiplexers) back to Chisel. This means that the code can only be partly mapped and the remainder will have to be reconstructed using some smart guessing.\\

\paragraph{Code Coverage with Verilator}
TODO\\

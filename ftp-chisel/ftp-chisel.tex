...
\todo{Decide on wording (HW, digital, testing, verification.)}
...
\item cocotb
\item ChiselTest~\cite{chisel:tester2}
...
that runs the hardware under test.
...
We will implement coverage inside of the FIRRTL execution engine Treadle using a technique
...
Synopsis, one of the three big EDA  companies, provides tool support for the SDH project.
WSA provides a use case a decimation filter written in VHDL that we plan to verify with our
Chisel/Scala  verification framework.

\todo{Add contributions from WSA, Syosil.}

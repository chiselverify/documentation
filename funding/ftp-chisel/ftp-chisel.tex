...
and different tools supportinf different subsets of the full specification.
...
RFuzz~\cite{rfuzz2018}, lead by researchers at UC Berkeley, focuses on ``coverage-guided fuzz mutational testing''. This method relies on FPGA-accelerated simulation.
RFuzz only generates random bit streams, our goal is to focus as well on the generation of coherent random
test vectors.
...
Martin has {\bf written the Chisel textbook}~\cite{chisel:book}\footnote{see
\url{http://www.imm.dtu.dk/~masca/chisel-book.html}}, which has been translated
...
{\bf Alceste Scalas} is associate professor at DTU Compute. His research interest is in formal methods and programming languages to aid software design, development, and verification. He has relevant experience with the Scala programming language and has been invited to the Scala World 2019 developers' conference.
...
lines of code between two implementations and test done in SystemVerilog/UVM and with Chisel/ChiselVerify.

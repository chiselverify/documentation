...
Most of us experienced the chip shortage crisis. 
The shortage has revealed the strong dependency of many industries on semiconductor chips.
The European Chip Act aims to strengthen Europe's resilience and competitiveness in
semiconductor technologies and applications in response to the shortage.
Furthermore, the greater Copenhagen area has many (15+) innovative chip design companies.
The companies expect a need for about 20 new engineers in chip design per year.
...
This project aims to support the Danish chip design industry by {\bf increasing
the productivity of chip design} and testing engineers.
...
to design and test digital systems.
...
from the last decades and explore their applicability to hardware design.}
We aim to {\bf raise the tooling level for a digital design to increase productivity}.
The time for verifying (testing) digital systems is about double the time of developing
...
VHDL and Verilog are the classic hardware description languages that first appeared in the 1980s. 
...
Recent advances with SystemVerilog and Chisel \cite{chisel:dac2012, chisel:book}
have brought object-oriented programming into digital design and verification.
...
behavior (successful termination or crash); code coverage can
...
It allows the creation of reusable test benches (i.e., using the same test for multiple designs)~\cite{uvm2015b}.
...
They transform the one-dimensional test input generated by AFL into a sequence of inputs applied
on sequential clock cycles.
During simulation, the tool detects bugs by checking invariants, described as assertions.
...
the languages and tools involved in the project and related work.
They will learn about the Scala programming language and ScalaCheck
(a Scala implementation of property-based testing) on the software side.
On the hardware side,
the researchers need to get familiar with Chisel, SystemVerilog, and
UVM.

\paragraph{Scientific Challenges}
The main scientific challenge is bringing software-based design and testing methods into
the hardware engineering field. The challenge is twofold: (1) to get classic
...
tests that trigger the properties, combined with fuzzing to increase the
overall testing coverage. We plan to extend ScalaCheck to generate tests
based on our temporal properties and add hardware-fuzzing-based
testing support to ChiselTest~\cite{chisel:tester2}. We will also take
inspiration from Google's ClusterFuzz~\cite{web:clusterfuzz} to explore the
distribution of hardware design fuzzing over multiple testing nodes on a
cloud.
...
We will develop an object-oriented and functional verification framework based on Scala hardware fuzzing.
...
showed that a test written with UVM was about 800 lines of code, whereas a Scala-based
...
A website will host the project documentation, the published papers, and the design's source code.
%We will provide unrestricted and cost-free digital access to all research and development results.
We will have a coordination workshop with the project partners in the middle of the project.
At the end of the project, we will organize a design and verification workshop, including a hands-on tutorial,
...
transition.
%
The following {\bf companies are interested in the SDH project} and support it with use cases.
...
student for six months at the University of Sussex. During this time, they will explore the design and
implementation of a specification language for property tests using temporal logic within the
testing framework.
...
{\bf Martin Schoeberl (MS)} is a professor at DTU Compute and is the PI.
...
UC Berkeley and SiFive, a silicon valley startup.


{\bf Alceste Scalas (AS)} is associate professor at DTU Compute. His research interests are in formal methods and programming languages to aid design, development, and verification. He has relevant experience with the Scala programming language and has been invited to present his research at the Scala World 2019 developers' conference.
...
{\bf Martin Berger (MB)} is a senior lecturer at the University of Sussex.
...
We assume a code reduction of 2/3 will translate to a factor of two, increasing productivity.

...
Two key techniques are \emph{fuzzing} and \emph{property-based testing}.
...
ChiselTest tries to enable testing best practices from software engineering.
Its lightweight syntax encourages writing targeted unit tests by making small tests easy.

The \textit{Universal Verification Methodology} (UVM) was created as a standardized way
of writing test-benches on top of SystemVerilog. 
...
One project, named RFuzz~\cite{rfuzz2018} and lead by researchers at UC Berkeley, focuses on ``coverage-guided fuzz mutational testing''. This method relies on FPGA-accelerated simulation and new solutions allowing for quick and deterministic memory resetting, to efficiently use fuzzing on digital circuits.
RFuzz only generates random bit streams, our goal is to focus as well on the generation of coherent random generated programs to test a processor.
...
Software fuzzing is based on the actual execution of the program under test; such a program can run at (near-)full speed, and can be easily distributed over thousands of testing nodes via cloud-based fuzzing tools like Google's ClusterFuzz \cite{web:clusterfuzz}.
We will add support for hardware fuzzing based testing to ChiselTest~\cite{chisel:tester2}.
We will develop property based testing, by exploring ScalaCheck and adding
temporal assertions to the DUT.
...
Huawei will follow the project within informal discussions and cooperate on an
advanced type system fur Chisel.
...
The project is divided into several tasks, each planned in person-months (PM).
%The Gantt chart shows the project schedule.
For an assessment of the project's success, we have following milestones:

...
compared to software development and testing.\footnote{Chris Lattner, The Golden Age of Compilers in an era of Hardware/Software co-design, invited talk at ASPLOS 2021.}
Within this project, we plan to
{\bf introduce software development and testing methods for digital design.}
...
We will use software development tools, languages, development, and testing methods
from the last decades and apply them to hardware design.
...
\paragraph{Hypothesis:} We hypothesize is that using tools, methods, and languages from the
...
behavior (successful termination or crash); code coverage
can guide the generation of random inputs \cite{takanen2018fuzzing,DBLP:journals/tse/BohmePR19,DBLP:conf/pldi/FuS17}.
...
Many implementations follow the idea from the seminal paper \cite{DBLP:conf/icfp/ClaessenH00}, including, e.g., ScalaCheck~\cite{ScalaCheck}.
...
It allows creating reusable test benches (i.e., using the same test for multiple designs)~\cite{uvm2015}.
...
Software fuzzing is based on the actual execution of the program under test; such a program can run at (near-)full
speed and be easily distributed over thousands of testing nodes via cloud-based fuzzing tools like Google's
...
We will express the properties of interest for a hardware design using temporal logic \cite{Baier2008} (e.g., Linear Temporal Logic, LTL \cite{Pnueli1977LTL}) or similar formalisms. A tool will translate those properties into
timed assertions for the testing.
Properties of interest can guide random test generation to cover a hardware design adequately.
...
{\small
  \begin{center}
    \begin{tabular}{lccp{110mm}l}
      ...
      TypeSys                &    1 + 1   &  AS + MB & Design a specification language for property tests using temporal logic. \\

...
\begin{abstract}
  Digital systems are a central part of our current and future digital enhanced live. Denmark has a considerable industry in the design and development of digital systems.\todo{such as...?} However, designing and verifying such digital systems becomes an ever-growing challenge. The project addresses: (1) how to simplify software developers' access to hardware accelerators in the cloud %how to make hardware accelerators in the cloud accessible for software developers,\todo{\#1 grammar issue; maybe rephrase?}% 
(2) how hardware verification can learn from software testing, and (3) co-verification of software and hardware with modern tooling.
...
and tools lack several decades of development. \note{How does one measure "decades" of development? Maybe use an other term.}
...
\note{Maybe shouldn't be as harsh on UVM. Try something like "unlike approaches that create complex reusable structures like UVM." }
...
The first challenge is how to determine the expected code outcome, also
...
The industrial project partners will learn and get exposed to modern tools and methods to facilitate the designing and testing of digital systems dramatically. Our partner companies will benefit from our advanced software tool that generates test stimuli achieving high coverage of the hardware in simulation. \todo{Can we be more concrete here?}
...
\note{Again, if this relates to chiselverify, then I don't think that it should be linked to UVM.}
...
\todo{Maybe also talk on mixed language co-verification, e.g., Python with cocotb}
...
The topic is relevant to several companies in Denmark. Following companies provided a commitment: Napatech (Jesper Birch), GN Hearing (Mark Brooks), Comcores (Thomas Noergaard, Joergen Carstensen), Teledyne Reson (Morten Rytter, Simon Andersen), WSAudiology (Ketil Julsgaard), Microchip (Thomas Aakjer), and SyoSil (Jacob Sander Andersen).


Following companies are considered additional partners: Synopsis (Martine Chegaray), and Demant (Anders Hebsgaard).
\todo{Missing replies.}
...
However, if you are still looking for industry participation to the project, then Napatech can join with the following intentions:

 * 750 hours in total over a period of 3 years.
 * Our primary interest is to improve design and  verification throughput using methods developed in the HaaS project.
